...
async function loadTeachersTable() {
  ...
  try {
    ...
    state.teacherAccounts = Array.isArray(list) ? list : [];
    ...
    const query = (document.getElementById('teacher-account-search')?.value || '').trim().toLowerCase();
    const selectedRole = document.getElementById('teacher-role-filter')?.value || '';
    const filteredTeachers = state.teacherAccounts.filter(t => {
      const searchable = `${t.username || ''} ${t.fullName || ''}`.toLowerCase();
      return (!query || searchable.includes(query)) && (!selectedRole || t.role === selectedRole);
    });

    if (filteredTeachers.length === 0) {
      teachersTableBody.innerHTML = '<tr><td colspan="4" class="text-center">ไม่พบบัญชีครูตามตัวกรอง</td></tr>';
      return;
    }
...
async function loadSystemLogs() {
  ...
  try {
    ...
    state.usageLogs = Array.isArray(logs) ? logs : [];
    ...
    const logQuery = (document.getElementById('analytics-log-search')?.value || '').trim().toLowerCase();
    const selectedRole = document.getElementById('analytics-role-filter')?.value || '';
    const selectedAction = document.getElementById('analytics-action-filter')?.value || '';
    const filteredLogs = state.usageLogs.filter(log => {
      const searchable = `${log.action || ''} ${log.role || ''} ${JSON.stringify(log.details || {})}`.toLowerCase();
      return (!logQuery || searchable.includes(logQuery)) &&
             (!selectedRole || log.role === selectedRole) &&
             (!selectedAction || String(log.action || '').toLowerCase().includes(selectedAction));
    });
...
async function loadSubjectsTable() {
  ...
  try {
    ...
    const searchQuery = (document.getElementById('subject-management-search')?.value || '').trim().toLowerCase();
    const departmentFilter = document.getElementById('subject-department-filter');
    const teacherFilter = document.getElementById('subject-teacher-filter');
    const selectedDepartment = departmentFilter?.value || '';
    const selectedTeacher = teacherFilter?.value || '';

    if (departmentFilter && departmentFilter.options.length <= 1) {
      [...new Set(list.map(item => item.Department || 'อื่นๆ / ไม่ระบุ'))].sort().forEach(department => {
        const option = document.createElement('option');
    ...
    if (teacherFilter && teacherFilter.options.length <= 1) {
      ...
      anyOption.value = 'any';
      anyOption.textContent = 'ครูทุกคน/กิจกรรม';
      teacherFilter.appendChild(anyOption);
    }

    const filteredSubjects = list.filter(subject => {
      const teacher = tList.find(item => item.username === subject.Teacher_Username);
      const searchable = `${subject.Subject_ID || ''} ${subject.Subject_Name || ''} ${subject.Department || ''} ${subject.Teacher_Username || ''} ${teacher?.fullName || ''}`.toLowerCase();
      return (!searchQuery || searchable.includes(searchQuery)) &&
             (!selectedDepartment || (subject.Department || 'อื่นๆ / ไม่ระบุ') === selectedDepartment) &&
             (!selectedTeacher || subject.Teacher_Username === selectedTeacher);
    });

    if (filteredSubjects.length === 0) {
      subjectsTableBody.innerHTML = '<tr><td colspan="5" class="text-center">ไม่พบรายวิชาตามตัวกรอง</td></tr>';
      return;
    }
...
async function loadAssignmentsTable() {
  ...
  try {
    ...
    const searchQuery = (document.getElementById('assignment-management-search')?.value || '').trim().toLowerCase();
    const subjectFilter = document.getElementById('assignment-subject-filter');
    const classFilter = document.getElementById('assignment-class-filter');
    const dueFilter = document.getElementById('assignment-due-filter')?.value || '';

    if (subjectFilter && subjectFilter.options.length <= 1) {
      state.subjects.forEach(subject => {
        const option = document.createElement('option');
        option.value = subject.Subject_ID;
        option.textContent = subject.Subject_Name || subject.Subject_ID;
    ...
    if (classFilter && classFilter.options.length <= 1) {
      ...
      list.forEach(assignment => {
        const values = Array.isArray(assignment.Class) ? assignment.Class : [assignment.Class];
    ...
    const permitted = list.filter(a => {
      if (!a || !hasSubjectAccess(a.Subject_ID)) return false;
      const searchable = `${a.Assignment_ID || ''} ${a.Assignment_Name || ''}`.toLowerCase();
      const assignedClasses = Array.isArray(a.Class) ? a.Class : [a.Class];
      const dueDate = a.Due_Date ? new Date(`${String(a.Due_Date).split('T')[0]}T00:00:00`) : null;
      let matchesDue = true;
      if (dueFilter && dueDate && !Number.isNaN(dueDate.getTime())) {
        if (dueFilter === 'overdue') matchesDue = dueDate < today;
        if (dueFilter === 'today') matchesDue = dueDate.getTime() === today.getTime();
        if (dueFilter === 'upcoming') matchesDue = dueDate > today;
      } else if (dueFilter && !dueDate) {
        matchesDue = false;
      }
      return (!searchQuery || searchable.includes(searchQuery)) &&
             (!selectedSubject || a.Subject_ID === selectedSubject) &&
             (!selectedClass || assignedClasses.includes(selectedClass)) &&
             matchesDue;
    });

    if (permitted.length === 0) {
      assignmentsTableBody.innerHTML = '<tr><td colspan="7" class="text-center">ไม่พบภาระงานตามตัวกรอง</td></tr>';
      return;
    }
...
function debounceFilter(callback, delay = 180) {
  let timeoutId;
  return () => {
...
const managementFilterBindings = [
  ['teacher-account-search', 'input', debounceFilter(loadTeachersTable)],
  ['teacher-role-filter', 'change', loadTeachersTable],
  ['subject-management-search', 'input', debounceFilter(loadSubjectsTable)],
  ['subject-department-filter', 'change', loadSubjectsTable],
  ['subject-teacher-filter', 'change', loadSubjectsTable],
  ['assignment-management-search', 'input', debounceFilter(loadAssignmentsTable)],
  ['assignment-subject-filter', 'change', loadAssignmentsTable],
  ['assignment-class-filter', 'change', loadAssignmentsTable],
  ['assignment-due-filter', 'change', loadAssignmentsTable],
  ['analytics-log-search', 'input', debounceFilter(loadSystemLogs)],
  ['analytics-role-filter', 'change', loadSystemLogs],
  ['analytics-action-filter', 'change', loadSystemLogs]
];

managementFilterBindings.forEach(([id, eventName, handler]) => {
  const element = document.getElementById(id);
  if (element) element.addEventListener(eventName, handler);
});

document.querySelectorAll('.btn-clear-management-filter').forEach(button => {
  button.addEventListener('click', () => {
    const group = button.dataset.filterGroup;
    const groups = {
      teachers: {
        ids: ['teacher-account-search', 'teacher-role-filter'],
        render: loadTeachersTable
      },
      subjects: {
        ids: ['subject-management-search', 'subject-department-filter', 'subject-teacher-filter'],
        render: loadSubjectsTable
      },
      assignments: {
        ids: ['assignment-management-search', 'assignment-subject-filter', 'assignment-class-filter', 'assignment-due-filter'],
        render: loadAssignmentsTable
      },
      analytics: {
        ids: ['analytics-log-search', 'analytics-role-filter', 'analytics-action-filter'],
        render: loadSystemLogs
      }
    };
